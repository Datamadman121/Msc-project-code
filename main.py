...
parser.add_argument('--decay_epoch', default=(5,9), type=eval, required=False, help='(default=%(default)d)')
...
parser.add_argument('--network_type', default=0, type=int, required=False, help='(default=%(default)d)')
...
def main():
    ...
    assert network_type in [0, 1, 2, 3]
    
    model = models.__dict__[args.approach](pretrained=True, num_classes=attr_num,network_type=network_type)
...
def save_checkpoint(state, epoch, prefix, filename='.pth.tar'):
    ...
    directory = "./my_models/" + args.experiment + '/' + args.approach + '/'
