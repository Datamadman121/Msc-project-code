...
from .attention import PixelAttention, ChannelAttention
...
class InceptionNet(nn.Module):
    def __init__(self, num_classes=51, network_type=0):
        ...
        self.channel_att = ChannelAttention(1024, 256)
        self.pixel_att = PixelAttention(1024, 256)
    ...
    def forward(self, input):
        ...
        if self.network_type == 0:
            ...
            channel_scale = self.channel_att(feat_5b)
            feat_5b = feat_5b * channel_scale
        elif self.network_type == 2:
            pixel_scale = self.pixel_att(feat_5b)
            feat_5b = feat_5b * pixel_scale
        elif self.network_type == 3:
            channel_scale = self.channel_att(feat_5b)
            feat_5b = feat_5b * channel_scale
            pixel_scale = self.pixel_att(feat_5b)
            feat_5b = feat_5b * pixel_scale
        # torch.Size([32, 320, 32, 16])b,c,h,w
        # torch.Size([32, 608, 16, 8])
        # torch.Size([32, 1024, 8, 4])
